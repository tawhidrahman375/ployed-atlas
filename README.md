...
- The one thing standing between "armed" and "actually sending" right now is simply that **0 of 59 queued leads currently have an email** (see the Hunter.io coverage note below) — as soon as a future run enriches one, it flows straight through with no further confirmation.

**Real finding on Hunter.io coverage**: in live testing, 0 of 10 real Apollo-sourced leads (small, newer agency founders — the actual ICP) had a match in Hunter's database. The only successful match was a deliberately well-known test case (Reid Hoffman), used purely to confirm the code path works. In practice, expect Apollo to keep finding good leads but Hunter to enrich very few of them with an email — most will need the manual X DM / LinkedIn comment path (not built yet) rather than ending up in Echo's queue at all. This is a real data-coverage limitation, not a bug to fix.

**What actually happens next**: the VPS cron runs `npm run morning` at 08:00 UTC daily. Each run, Apollo finds a handful of new leads and Hunter attempts to enrich up to 5. The first time one succeeds, Echo writes real cold email copy and Instantly sends it — automatically, on schedule, with no per-email confirmation from anyone. If you want another look before that first real send happens, keep an eye on the dashboard's "Today's numbers" (emails sent) and the risk flags section.
