...
- The one thing standing between "armed" and "actually sending" right now is simply that **0 of 59 queued leads currently have an email** (see the historical Hunter.io coverage note above) — as soon as a future run enriches one, it flows straight through with no further confirmation. As of 2026-08-08 this also needs `ANYMAIL_FINDER_API_KEY` set before enrichment produces anything at all — see the provider-switch note above.

**What actually happens next**: the VPS cron runs `npm run morning` at 08:00 UTC daily. Each run, Apollo finds a handful of new leads and Anymail Finder attempts to enrich up to 5. The first time one succeeds, Echo writes real cold email copy and Instantly sends it — automatically, on schedule, with no per-email confirmation from anyone. If you want another look before that first real send happens, keep an eye on the dashboard's "Today's numbers" (emails sent) and the risk flags section.
