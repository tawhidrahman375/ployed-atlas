...
**2026-08-22: swapped Instantly for Saleshandy** as Echo/Pulse/Sentinel's cold-email sender (`src/lib/saleshandy.ts`, replacing `src/lib/instantly.ts`). `SALESHANDY_API_KEY`/`SALESHANDY_SEQUENCE_ID` replace `INSTANTLY_API_KEY`/`INSTANTLY_CAMPAIGN_ID`. **Not yet verified live** — this session's network egress couldn't reach any Saleshandy domain (docs or API) to confirm exact endpoint paths and response field names, so `saleshandy.ts` is marked with `⚠️` at every guessed field/path. Treat every "verified live" claim below about Instantly-era behavior as historical, describing the old integration, not a current guarantee about Saleshandy.

## What's real vs stubbed right now

As of 2026-08-08 (pre-Saleshandy-swap), every agent is real, tested code — not just plumbing. Confirmed working end-to-end with live credentials (not just typechecked): Nova (real YouTube transcripts → Claude extraction), Vega (real competitor page diffing + Reddit RSS), Pulse (real GSC/PostHog/Instantly pulls — Instantly since replaced by Saleshandy, unverified), Muse (real content uploaded to Supabase Storage with public URLs), Sentinel (real Instantly bounce-rate check — same caveat), Ledger (real Stripe MRR reads). Echo is real, working code too, but see the gate below.
...
**Echo is deliberately gated.** `pushToSaleshandy()` is implemented (adds a prospect to a real Saleshandy sequence via their API — see the 2026-08-22 swap note above, unverified live), and it requires `SALESHANDY_SEQUENCE_ID`, which is *not* set anywhere by default. Nothing will send a real email until you've created an actual sequence in Saleshandy and chosen to set that variable yourself — that's an intentional extra step, not an oversight.
...
| Pulse | 🟡 wired, unverified | GSC (service account JWT), PostHog (HogQL query API) real; Saleshandy sequence analytics unverified since the 2026-08-22 provider swap |
...
| Sentinel | 🟡 wired, unverified | 7-day bounce-rate window against Saleshandy sequence analytics, unverified since the 2026-08-22 provider swap. Enforcement logic itself (Echo checks for an unresolved red flag tagged `agent: 'Echo'` before sending) was verified live pre-swap and is unchanged |
| Ledger | ✅ real | Reads live MRR from Stripe (`sk_live_...` key); webhook *listener* still isn't deployed anywhere (see step 3) |
| Echo | 🟡 wired, unverified | Needs `SALESHANDY_SEQUENCE_ID` set on purpose before it can send anything live; Saleshandy push itself unverified since the 2026-08-22 provider swap |
...
4. 🟡 **Pulse** — GSC (service account JWT via `google-auth-library`) and PostHog (HogQL query API) pulling real data into `dashboard_metrics`; Saleshandy sequence analytics unverified since the 2026-08-22 provider swap.
...
6. 🟡🔒 **Echo** — `pushToSaleshandy()` implemented against Saleshandy's leads API, unverified live since the 2026-08-22 provider swap (see the note near the top of this file). Gated behind `SALESHANDY_SEQUENCE_ID`, which is intentionally unset — see the autonomy note at the bottom.
...
11. 🟡 **Forge / Sentinel** — Forge's table plumbing works (nothing calling `proposeExperiment()` yet — that's a "when you want to run an experiment" trigger, not a startup task). Sentinel's bounce-rate check reads 7 days of sequence analytics, now via Saleshandy and unverified since the 2026-08-22 provider swap. The enforcement logic itself is unchanged and was verified live pre-swap: Echo refuses to send if an unresolved red flag tagged `agent: 'Echo'` exists (planted a real flag, confirmed Echo sent 0, removed it). There's no UI yet to mark a flag resolved once the underlying issue is fixed — do it directly in Supabase (`risk_flags.resolved = true`).
...
**Runner (VPS) — `.env`:**
`ANTHROPIC_API_KEY`, `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `BRAVE_SEARCH_API_KEY`, `ANYMAIL_FINDER_API_KEY`, `SALESHANDY_API_KEY`, `SALESHANDY_SEQUENCE_ID` (leave unset until you're ready for Echo to send real emails), `PEXELS_API_KEY`, `COMPOSIO_API_KEY`, `POSTHOG_API_KEY`, `POSTHOG_PROJECT_ID`, `GSC_SITE_URL`, `GSC_CREDENTIALS_JSON`, `PRODUCTHUNT_API_TOKEN`
...
**Status: re-gated.** Everything below described the old Instantly campaign (`cfac9833-addf-4a2d-a994-d1293c7ea67f`, "My Campaign", status Active) as of 2026-08-08 — that campaign and its `INSTANTLY_CAMPAIGN_ID` gate no longer apply after the 2026-08-22 Saleshandy swap (see the note near the top of this file). The equivalent gate is now `SALESHANDY_SEQUENCE_ID`, which is **not set anywhere by default** — the send path is back to fully un-armed until you deliberately create a real sequence in Saleshandy and set that variable yourself, both locally and on the VPS.

- Echo checks for an unresolved red risk flag (Sentinel's bounce-rate alarm) before sending anything, and refuses if one exists — this enforcement logic is unchanged by the provider swap and was verified live pre-swap, but Sentinel's bounce-rate read itself now goes through Saleshandy and is unverified (see `saleshandy.ts`).
- `pushToSaleshandy()` itself hasn't been exercised against a real Saleshandy account from this environment (network egress to every Saleshandy domain is blocked here) — smoke-test one real prospect add before trusting this end to end.
- Separately, enrichment is still the practical bottleneck: leads without an email never reach Echo at all (see the historical Hunter.io/Anymail Finder coverage notes above) — that constraint is independent of the ESP and unaffected by this swap.

**What actually happens next, once re-armed**: the VPS cron runs `npm run morning` at 08:00 UTC daily. Each run, Apollo finds a handful of new leads and Anymail Finder attempts to enrich up to 5. The first time one succeeds, Echo writes real cold email copy and pushes it to Saleshandy — automatically, on schedule, with no per-email confirmation from anyone, once `SALESHANDY_SEQUENCE_ID` is set. If you want another look before that first real send happens, keep an eye on the dashboard's "Today's numbers" (emails sent) and the risk flags section.
